...
![](https://3553248446-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M5HOStxvx-Jr0fqZhyW%2F-MPoEIw5NEdxkZWUDr9T%2F-MPoEkmmUQuqieC8v02g%2F111.jpg?alt=media&token=022e058e-4545-4f25-b3dd-dec086947219)

이런 함수를 구체적인 리스트에 대해 호출 할 때 타입 인자를 명시적으로 지정할 수 있다.
...
#### (2) 타입 파라미터 제약
어떤 타입을 제네릭 타입의 타입 파라미터에 대한 상한(upper bound)으로 지정하면 그 제네릭 타입을 인스턴스화 할 때 사용하는 타입 인자는 반드시 그 상한 타입이거나 그상한 타입의 하위 타입이어야 한다.

![](https://3553248446-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M5HOStxvx-Jr0fqZhyW%2F-MPoEIw5NEdxkZWUDr9T%2F-MPoEonLUtayUrkJdpEj%2F222.jpg?alt=media&token=abfeb304-7ea7-45eb-a62d-94e54bf03453)

타입 파라미터  T에 대한 상한을 정하고 나면 T 타입의 값을 그 상항 타입의 값으로 취급할 수 있다.
...
==<T:Any> 라는 제약은 T 타입이 항상 널이 도리수 없는 타입이 되도록 보장한다.==
...
또는 타입 파라미터를 널이 될 수 없는 타입으로 제약하기만 하면 타입 인자로 널이 될 수 있는 타입이 들어오는 일 막을 수 있다.
...
![](https://3553248446-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M5HOStxvx-Jr0fqZhyW%2F-MPoEIw5NEdxkZWUDr9T%2F-MPoEs9-y3wNso36Mo_I%2F333.jpg?alt=media&token=3b77c3ae-341a-4cf5-84c0-6356929ed883)
...
#### (2) 공변성 : 하위 타입 관계를 유지
`Producer<T>`를 예로 공변성 클래스를 설명하자. A가 B의 하위 타입일 때 `Producer<A>`가 `Producer<B>`의 하위 타입이면 Producer 공변적이다. 이를 하위 타입 관계가 유지된다고 말한다. 예를 들어 Cat가 Animal의 하위 타입이기 때문에 `Producer<Cat>`은 `Producer<Animal>`의 하위 타입이다.
...
![](https://3553248446-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M5HOStxvx-Jr0fqZhyW%2F-MPoEIw5NEdxkZWUDr9T%2F-MPoF7rlncnxU3Jpl97x%2F444.jpg?alt=media&token=5e4377a2-46c4-4393-a156-731a8154e0d0)



#### (3) 스타 프로젝션: 타입인자 대신 * 사용
앞부분에서 타입 검사와 캐스트에 대해 설명할 때 제네릭 타입 인자 정보가 없음을 표현하기 위해 스타 프로젝션을 사용한다고 말했다. 이제 스타 프로젝션의 의미를 자세히 살펴보자

첫째, MutableList <>는 MutableList<Any?>와 같지 않다.MutableList<Any?>는 모든 타입의 원소를 담을 수 있다는 사실을 알 수 있는 리스트다. 반면 MutableList<> 는 어떤 정해진 구체적인 타입의 원소만을 담는 리스트지만 그 원소의 타입을 정확히 모른다는 사실을 표현한다.
...
```kotlin
package io.justdoit.prop.global.controller  
  
import java.util.*  
  
  
fun main(){  
    val randomGreeting1 = random1("hello", "willkomment", "bonjour")  
    val randomGreeting2 = random2("hello", "willkomment", "bonjour")  
  
    println(randomGreeting2.length)  
  
    //컴파일러가 타입을 알수가 없어서 에러 난다.  
    println(randomGreeting1.length)}  
