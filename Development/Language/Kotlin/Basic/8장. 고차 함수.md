...
![](https://3553248446-files.gitbook.io/~/files/v0/b/gitbook-legacy-files/o/assets%2F-M5HOStxvx-Jr0fqZhyW%2F-MPZjfPG3EVetS_xZjcV%2F-MPZjxP-ctiup-hgbclx%2F8-1.jpg?alt=media&token=a72bfe0f-a562-4605-a56e-948f37a42178)


Unit 타입은 의미 있는 값을 반환하지 않는 함수 반환 타입에 쓰는 특별한 타입이다. (자바의 Void) 그냥 함수를 정의한다면 함수의 파라미터 목록 뒤에 오는 Unit 반환 타입 지정을 생략해도 되지만, 함수 타입을 선언할 때는 반환 타입을 반드시 명시해야 하므로 Unit을 빼먹어서는 안 된다.
...
val test: (SiteVisit)->Double = SiteVisit::duration  
val test2: (SiteVisit)->Double = {it.duration}
```
...
#### (1) 람다 안의 return문: 람다를 둘러싼 함수로 부터 반환
